function [estFin,optFin,EMoptFin,estHist]...
    = BiGAMP_EM(Y,problem,BiGAMPopt, PrioriIn, EMopt)
%refined by JW ZHANG, use avg spar as the stop condition
...
defaultOptions.sig_dim = 'row'; %learn a single variances for X entries (joint)
...
if ~isfield(EMopt,'noise_var') || isempty(EMopt.noise_var)
    if ~BiGAMPopt.sparseMode
        EMopt.noise_var = PrioriIn.noiseVar;%sum(sum(abs(Y(omega)).^2))/(sum(sum(omega))*101); ******
    else
...
if ~isfield(EMopt,'active_var') || isempty(EMopt.active_var)
    nuX = PrioriIn.nuX;%(norm(Y,'fro')^2/numel(omega) - nuw)/N/mean(lambda(:)); %*****PrioriIn.nuX;%
else
...
%             num2str(drawAttempts)])
    ...
    elseif strcmp(EMopt.init,'random') %or random
        ...
        B1 = randi([0,1],sqrt(length(Sam))*M,N); 
        Ahat = Constell_Modulate(B1, Sam);
...
while stop < 2
  ...
  %  gX = CAwgnEstimIn(meanX, nuX);
    ...
    %Run BiG-AMP
    %[estFin2,~,estHist2,state] = BiGAMP(gX, gA, gOut, problem, BiGAMPopt,state);
    [estFin2,~,estHist2,state] = BiGAMP_test(gX, gA, gOut, problem, BiGAMPopt,state, PrioriIn); % state evolution test 
...
%     AvgSpar = sum(sum(p1))/L;
%     if abs(AvgSpar-AvgSparOpt)/AvgSpar<1e-4
%         flagRank = 1;
%     else
%         AvgSparOpt = AvgSpar;
%     end
    ...
    norm_change = abs(norm(zhat-zhatOld,'fro'))^2/abs(norm(zhat,'fro'))^2;
    ...
    if EMopt.learn_noisevar
        ...
        if BiGAMPopt.sparseMode
            nuw = abs(norm(Y - zhat,'fro'))^2 ...
                /numel(omega);
        else
            
            nuw = abs(norm(Y(omega) - zhat(omega),'fro'))^2 ...
                /numel(omega);
        end
...
%%    J£¨t£©for result choose
        KLMSE = abs(norm(Y - zhat,'fro'))^2;  
    %  KLMSE = mean(lambda(:)) ;
        if(KLMSE < vall)
            vall = KLMSE;
            %Update finals
