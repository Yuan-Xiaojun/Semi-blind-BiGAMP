classdef AwgnEstimIn < EstimIn
    ...
    methods
        ...
        function [Ahat, Avar, val] = estim(obj, qhat, qvar, Sam, D, P_num)
            % Get prior
            uhat0 = obj.mean0;
            uvar0 = obj.var0; 
...
%             Avar = gain.*qvar;
 %% Blind  Compute posterior mean and variance
...
%   Compute marginal probability
         for i=1:length(Sam)      
              L = abs(Sam(1,i)*ones(M,N)-qhat).^2./qvar;
              Pa(:,:,i)=exp(-L);      
         end      
         Pa = max(Pa,1e-7);
         P = sum(Pa, 3);
...
%%
       % below  for semi-blind      
          
            Pa = SCSE(Pa, P_num);     %  add pilots consider phase ambiguity
          %  Pa = S-SCSE(Pa, P_num);     %  add pilots consider phase and permutation ambiguity
...
%%
            % Compute posterior mean and variance  
               for i=1:length(Sam)
                   Ahat11 = Pa(:,:,i).*(Sam(i)*ones(M,N));
                   Ahat = Ahat+Ahat11;              
               end
               for i=1:length(Sam)
                   Avar11 = Pa(:,:,i).*(abs(Sam(i)*ones(M,N)-Ahat).^2);
                   Avar = Avar+Avar11; 
               end
%% training                 
%             Ahat(1:r,:) = D;         %  Training based
%             Avar(1:r,:) = 0 ;
                ...
                if ~(obj.maxSumVal)
                   ...
                   xvar_over_uvar0 = qvar./(uvar0+qvar);
                    val = (log(xvar_over_uvar0) + (1-xvar_over_uvar0) ...
                     ...
                     %  val = - (Pa(:,:,1).*log(4*Pa(:,:,1)) + Pa(:,:,2).*log(4*Pa(:,:,2)) + Pa(:,:,3).*log(4*Pa(:,:,3)) + Pa(:,:,4).*log(4*Pa(:,:,4)));
                else
